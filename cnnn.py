...
#-----------------构建网络----------------------
#占位符
x=tf.compat.v1.placeholder(tf.compat.v1.float32,shape=[None,w,h,c],name='x')
y_=tf.compat.v1.placeholder(tf.compat.v1.int32,shape=[None,],name='y_')
print("开始构建网络")
...
def inference(input_tensor, train, regularizer):
    with tf.compat.v1.variable_scope('layer1-conv1'):
        ...
        conv1 = tf.compat.v1.nn.conv2d(input_tensor, conv1_weights, strides=[1, 1, 1, 1], padding='SAME')
    ...
    with tf.compat.v1.variable_scope("layer5-conv3"):
        ...
        relu3 = tf.compat.v1.nn.relu(tf.compat.v1.nn.bias_add(conv3, conv3_biases))
    ...
    with tf.compat.v1.variable_scope("layer7-conv4"):
        ...
        relu4 = tf.compat.v1.nn.relu(tf.compat.v1.nn.bias_add(conv4, conv4_biases))
    ...
    with tf.compat.v1.variable_scope('layer10-fc2'):
        ...
        fc2 = tf.compat.v1.nn.relu(tf.compat.v1.matmul(fc1, fc2_weights) + fc2_biases)
        if train: fc2 = tf.compat.v1.nn.dropout(fc2, 0.5)

    with tf.compat.v1.variable_scope('layer11-fc3'):
        fc3_weights = tf.compat.v1.get_variable("weight", [512, 15],
                                      initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.1))
        if regularizer != None: tf.compat.v1.add_to_collection('losses', regularizer(fc3_weights))
        fc3_biases = tf.compat.v1.get_variable("bias", [15], initializer=tf.compat.v1.constant_initializer(0.1))
        logit = tf.compat.v1.matmul(fc2, fc3_weights) + fc3_biases

    return logit

#---------------------------网络结束---------------------------
regularizer = tf.compat.v1.keras.regularizers.l2(0.0001)
logits = inference(x,False,regularizer)

#(小处理)将logits乘以1赋值给logits_eval，定义name，方便在后续调用模型时通过tensor名字调用输出tensor
b = tf.compat.v1.constant(value=1,dtype=tf.compat.v1.float32)
logits_eval = tf.compat.v1.multiply(logits,b,name='logits_eval')

loss=tf.compat.v1.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=y_)
train_op=tf.compat.v1.train.AdamOptimizer(learning_rate=0.001).minimize(loss)
correct_prediction = tf.compat.v1.equal(tf.compat.v1.cast(tf.compat.v1.argmax(logits,1),tf.compat.v1.int32), y_)
acc= tf.compat.v1.reduce_mean(tf.compat.v1.cast(correct_prediction, tf.compat.v1.float32))
